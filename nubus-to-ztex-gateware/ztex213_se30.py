...
_se30_pds_io = [
    ...
    # CPUCLK and ECLK removed to free pins for SD Card (V1, T4)
    # ("cpu_clk_3v3_n",      0, Pins("V1"),  IOStandard("lvttl")),
    # ("eclk_3v3_n",         0, Pins("T4"),  IOStandard("lvttl")),
    ...
    # Extra: G17 -> 30 pins
    # Extra: H17 -> 31 pins
    # Extra: B11 (Was USB DP) -> 32 pins (Replaces L3 which was HDMI)
    ...
    ("pds_d_3v3_n",        0, Pins(
        ...
        "G17 " +                  # RESET (1) (Note: G17 is used as Data bit here, not Reset signal)
        "H17 " +                  # MASTER_DIR (1)
        "B11"                     # EXTRA (1) - Was USB DP (Replaces L3)
    ), IOStandard("lvttl")),
    ...
    # Reset
    # Mapped to A11 (Was USB DM) to avoid conflict with SD Card D3 (U4)
    ("reset_3v3_n",        0, Pins("A11"), IOStandard("lvttl")),
]

_se30_peripherals = [
    # USB - DISABLED (Pins used for PDS Data and Reset)
    # ("usb", 0,
    ...
    # Enabled now that conflicts are resolved
    ("hdmi", 0,
...
class SE30Platform(ZTexPlatform):
    def __init__(self, variant="ztex2.13a"):
        ...
        # We pass empty list as we define pins explicitly.
